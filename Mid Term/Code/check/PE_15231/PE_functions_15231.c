...
void read_to_crs(const char *filename,int **pp1, int**pp2, float **pp3, int **pp4, int *Nodes,int *tot_link,int *number_dangling)
{
    ...
    *number_dangling=0;
    ...
    while (fscanf(fp,"%d %d",&n1,&n2)==2)
    {
      ...
      else
      {
        ...
        {
          j+=1;}
        if(j>inbound[n1]){printf("Warning: Faulty behaviour read_to_crs function under column count! \n");} // give watning if element is assigned out of node bounds
        col[min_index+j]=n2;
      }
    }
    ...
    float *rows=(float*)calloc(*tot_link,sizeof(float)); //values

    for(int j=0;j<*tot_link;j++)
    {
      rows[col[j]]=rows[col[j]]+val[j]; //sum all rows
    }
    ...
    *number_dangling=0;
    for(int i=0;i<*Nodes;i++)
    {
      if (rows[i]==0) //if any rows sum to zero count one dangling page
      {
        *number_dangling= *number_dangling+1;
      }
    }

    dangling=(int*)calloc(*number_dangling,sizeof(int)); //construct array to hold index
    int d_counter =0;
    for(int i=0;i<*Nodes;i++)
    {
      if (rows[i]==0) //if any rows sum to zero store index of node as dangling node and
      {
...
void top_n_webpages(int n, int *Nodes,float *PageRank,int **pp1,float **pp2)
{
  int *index_topranked;
  float *toprank;
  index_topranked=(int*)calloc(n,sizeof(int)); //array holding page rank of pages
  toprank=(float*)calloc(n,sizeof(float)); //array holding page rank of pages


  struct str objects[*Nodes];
  for (int i = 0; i < *Nodes; i++) //place Pagrank value and index of node in structure
  {
      objects[i].value = PageRank[i];
      objects[i].index = i;
  }

  qsort(objects, *Nodes, sizeof(objects[0]), cmp); //sort on value using qsort (wish I knew of this function earlier)

  for (int i = 0; i <n; i++) //cast index and value from struct into arrays for the top n ranks
    {
      index_topranked[i]=objects[i].index;
      toprank[i]=objects[i].value;
    }
...
free(toprank);
} // end of top_n_webpages function
